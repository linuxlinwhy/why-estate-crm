...
import {
  ...
  AlertCircle, CheckCircle2, Loader2, ArrowLeft, LayoutGrid, Users, FileText,
} from 'lucide-react';
...
// Initial demo boards matching Malaysian property context
const INITIAL_BOARDS: Board[] = [
  { id: 'board_1', name: 'Millerz Square @ OKR',         color: '#F97316', createdAt: Date.now() - 5 },
  { id: 'board_2', name: 'AKASA @ Cheras',               color: '#F97316', createdAt: Date.now() - 4 },
  { id: 'board_3', name: 'The Rainz @ Bukit Jalil',      color: '#F97316', createdAt: Date.now() - 3 },
  { id: 'board_4', name: 'Nidoz Residence @ Desa Petaling', color: '#8B5CF6', createdAt: Date.now() - 2 },
  { id: 'board_5', name: 'Bukit Jalil | All Condo',      color: '#1EC9C4', createdAt: Date.now() - 1 },
  { id: 'board_6', name: 'OKR South | All Condo',        color: '#1EC9C4', createdAt: Date.now() },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.45)' }}>
      <div className="bg-white rounded-2xl shadow-2xl w-[420px] overflow-hidden" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div className="flex items-start justify-between px-6 pt-5 pb-4">
          <div>
            <h3 className="text-lg font-bold" style={{ color: '#1A202C' }}>New Board</h3>
            <p className="text-xs mt-0.5" style={{ color: '#9CA3AF' }}>Create a prospect board</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-100 mt-0.5"><X size={16} className="text-gray-400" /></button>
        </div>
        ...
        <div className="px-6 pb-5 space-y-5">
          ...
          {/* Preview */}
          <div>
            <label className="text-xs font-semibold mb-2 block" style={{ color: '#374151' }}>Preview</label>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t border-gray-100" style={{ background: '#F8FAFB' }}>
          <button onClick={onClose} className="px-4 py-2 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
            Cancel
          </button>
...
// ─── Board View ───────────────────────────────────────────────────────────────
function BoardView({ boards, onSelectBoard, onAddBoard, prospectsByBoard }: {
  boards: Board[];
  onSelectBoard: (board: Board) => void;
  onAddBoard: () => void;
  prospectsByBoard: Record<string, { total: number; positive: number; listings: number }>;
}) {
  return (
    <div className="h-full flex flex-col" style={{ background: '#18181B' }}>
      {/* Top bar */}
      <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
        style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div>
          <h2 className="text-lg font-bold text-white">Prospect Hub</h2>
          <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>{boards.length} board{boards.length !== 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={onAddBoard}
      ...
      <div className="flex-1 overflow-auto px-6 py-5">
        <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}>
          {boards.map((board) => {
            const stats = prospectsByBoard[board.id] ?? { total: 0, positive: 0, listings: 0 };
            const initials = board.name.trim().split(/\s+/).slice(0, 2).map((w) => w[0].toUpperCase()).join('');
            return (
              <button
                key={board.id}
                onClick={() => onSelectBoard(board)}
                className="text-left rounded-2xl p-4 transition-all duration-200 hover:scale-[1.03] hover:brightness-110 active:scale-[0.98]"
                style={{ background: board.color, minHeight: 120 }}>
                {/* Board name */}
                <div className="mb-4">
                  <p className="text-sm font-bold text-white leading-snug line-clamp-2">{board.name}</p>
                </div>
                {/* Stats row */}
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg"
                    style={{ background: 'rgba(0,0,0,0.18)' }}>
                    <Users size={11} className="text-white opacity-80" />
                    <span className="text-xs font-semibold text-white">{stats.total}</span>
                  </div>
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg"
                    style={{ background: 'rgba(0,0,0,0.18)' }}>
                    <FileText size={11} className="text-white opacity-80" />
                    <span className="text-xs font-semibold text-white">{stats.listings}</span>
                  </div>
                  {stats.positive > 0 && (
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg ml-auto"
                      style={{ background: 'rgba(0,0,0,0.18)' }}>
                      <span className="w-1.5 h-1.5 rounded-full bg-green-300" />
                      <span className="text-xs font-semibold text-white">{stats.positive}</span>
                    </div>
                  )}
                </div>
              </button>
          ...
          <button
            onClick={onAddBoard}
            className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed transition-all duration-200 hover:border-[#1EC9C4] hover:bg-white/5 group"
            style={{ minHeight: 120, borderColor: 'rgba(255,255,255,0.15)' }}>
            <div className="w-9 h-9 rounded-full flex items-center justify-center transition-colors"
              style={{ background: 'rgba(255,255,255,0.08)' }}>
              <Plus size={18} className="text-white/40 group-hover:text-[#1EC9C4] transition-colors" />
            </div>
            <span className="text-xs font-medium text-white/30 group-hover:text-[#1EC9C4] transition-colors">New Board</span>
          </button>
        </div>
      </div>
    </div>
...
export default function ProspectHub() {
  ...
  // Prospects stored per board: boardId → Prospect[]
  const [boardProspects, setBoardProspects] = useState<Record<string, Prospect[]>>({
    board_1: seedProspects.slice(0, 4),
    board_2: seedProspects.slice(4, 9),
    board_3: seedProspects.slice(9, 13),
    board_4: seedProspects.slice(13, 16),
    board_5: seedProspects.slice(16, 18),
    board_6: [],
  });

  // Stats per board for the board cards
  const prospectsByBoard: Record<string, { total: number; positive: number; listings: number }> = {};
  boards.forEach((b) => {
    const prospects = boardProspects[b.id] ?? [];
    prospectsByBoard[b.id] = {
      total: prospects.length,
      positive: prospects.filter((p) => p.callingStatus === 'Positive').length,
      listings: prospects.filter((p) => p.listingType).length,
    };
  });

  const createBoard = (name: string, color: string) => {
    const newBoard: Board = { id: `board_${Date.now()}`, name, color, createdAt: Date.now() };
    setBoards((prev) => [...prev, newBoard]);
    setBoardProspects((prev) => ({ ...prev, [newBoard.id]: [] }));
  };

  // If no board selected → show board grid
  if (!activeBoard) {
    return (
      <>
        <BoardView
          ...
          prospectsByBoard={prospectsByBoard}
        />
        {showNewBoard && (
  ...
  // Rows for the active board
  const currentRows    = boardProspects[activeBoard.id] ?? [];
  const setCurrentRows = (updater: Prospect[] | ((prev: Prospect[]) => Prospect[])) => {
...
// ─── Prospect Table (previously the whole ProspectHub) ────────────────────────
function ProspectTable({ board, rows, setRows, onBack }: {
  board: Board;
  rows: Prospect[];
  setRows: (updater: Prospect[] | ((prev: Prospect[]) => Prospect[])) => void;
  onBack: () => void;
}) {
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col" style={{ background: '#F5F7FA' }}>
      ...
      <div className="flex items-center gap-2.5 px-4 py-3 bg-white border-b border-gray-100 flex-shrink-0 flex-wrap">
        {/* Back button + board breadcrumb */}
        <button onClick={onBack}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition-colors flex-shrink-0"
          style={{ color: '#6B7280' }}>
          <ArrowLeft size={13} />
          <LayoutGrid size={13} />
          Boards
        ...
        <div className="flex items-center gap-2 mr-1 flex-shrink-0">
          <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ background: board.color }} />
          <div>
            <h2 className="text-base font-bold leading-tight" style={{ color: '#2B3340' }}>{board.name}</h2>
            <p className="text-xs leading-none mt-0.5" style={{ color: '#A1A9B6' }}>{filtered.length} of {rows.length} records</p>
          </div>
        </div>
